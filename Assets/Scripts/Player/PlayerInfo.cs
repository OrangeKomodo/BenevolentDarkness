...
public class PlayerInfo : LivingEntity {
	...
	public PlayerQuickSave playerQuickSave = new PlayerQuickSave ();
				...
				if (currentItem.type.ToString ().Equals ("missionItem")) {
					if (Input.GetButtonDown ("Use")) {
						...
						if (currentItem.itemName.Equals ("MacGuffin")) {
							GameObject.FindGameObjectWithTag ("GameController").GetComponent<ObjectiveSystem> ().SetObjectiveStatus (101, Objective.Status.completed);
	...
	public void Attack (int typeOfAttack) {
		//0 = attack, 1 = knockout
		spellcaster.EndSpell(spellcaster.currentSpell); //WITH MIMIC THIS GOES FROM THE SPELLCASTER TO THE MIMIC PREFAB BACK TO THE PLAYER (INDISGUSE FUNCTION) THEN TO THE PLAYER CONTROLLER
