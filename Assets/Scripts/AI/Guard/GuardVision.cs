...
public class GuardVision : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (visibleGuards.Count > 0) {
			for (int i = 0; i < visibleGuards.Count; i++) {
				...
				if ((fellowGuard.state == Guard.State.dead || fellowGuard.state == Guard.State.unconscious) && !incapacitatedGuards.Contains (fellowGuard) && fellowGuard.transform.parent.parent == transform.parent.parent.parent) {
	...
	void OnTriggerEnter2D(Collider2D collider){
		if (collider.tag.Equals ("Player")) {
			if (collider.Equals (player.GetComponent<BoxCollider2D> ()))
				...
				circleVisible = true;
			inRange = GetComponent<CircleCollider2D> ().IsTouching (player.GetComponent<BoxCollider2D> ()) || GetComponent<CircleCollider2D> ().IsTouching (player.GetComponent<CircleCollider2D> ());
		}
	...
	void OnTriggerExit2D(Collider2D collider){
		if (collider.tag.Equals ("Player")) {
			if (collider.Equals (player.GetComponent<BoxCollider2D> ()) && !GetComponent<PolygonCollider2D> ().IsTouching (player.GetComponent<BoxCollider2D> ()))
				boxVisible = false;
			else if (collider.Equals (player.GetComponent<CircleCollider2D> ()))
				circleVisible = false;
		}
