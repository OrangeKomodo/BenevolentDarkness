...
[System.Serializable]
public class GuardQuickSave {

	public Vector2 position;
	public float direction;
	public Transform nextStop;
	public float health;
	public Guard.State state;
...
public class Guard : LivingEntity {
	...
	public GuardQuickSave guardQuickSave;
		...
		if (!playerDead && state != State.corrupted) {
			if (state == State.dead || state == State.unconscious) {
				...
				if (rb.gravityScale > 0f) {
					Debug.DrawRay (transform.position, -Vector2.up * (transform.parent.localScale.y * 1.25f), Color.green);
					if (Physics2D.Raycast (transform.position, -Vector2.up, transform.parent.localScale.y * 1.25f, whatIsGround)) {
						rb.gravityScale = 0f;
						rb.velocity = Vector2.zero;
				...
				if (!inForceField) {
					if (state == State.unaware) {
						if (!idling) {
							...
							rb.velocity = new Vector2 (GetDirection (transform.position, nextStop.position) * walkSpeed, 0f);
			...
			if (closestGuard != transform) {
				...
				if (closestGuardState != State.corrupted && closestGuardState != State.unconscious && closestGuardState != State.dead) {
	...
	void Idle () {
		...
		transform.localScale = new Vector3 (Mathf.Abs (transform.localScale.x) * idleDirection.x, transform.localScale.y, 0);
	...
	void ChangeDirection () {
		...
		transform.localScale = new Vector3 (Mathf.Abs (transform.localScale.x) * newDirection, transform.localScale.y, 0);
	}
	...
	float GetDirection (Vector3 currentPosition, Vector3 targetPosition){
		return targetPosition.x - currentPosition.x == 0f ? 1f : Mathf.Round (Mathf.Abs (targetPosition.x - currentPosition.x) / (targetPosition.x - currentPosition.x));
	}
	...
	void PlayerAwareness () {
		if (!playerDead && !inStasis && !inForceField && state != State.corrupted) {
			...
			if (state == State.suspicious && suspicionPercentage < 0.5f || state == State.alerted && suspicionPercentage == 0) {
				state = State.unaware;
				//anim.SetInteger ("Walk State", 1);
				canSpot = true;
			}
			...
			if (suspicionPercentage == 1f) {
                if (state != State.chasing && !guardFound) {
                    if (canSpot) {
                        GameObject.FindGameObjectWithTag("GameController").GetComponent<MoralitySystem>().timesSpotted++;
	...
	public void PlayerInMeleeRange (bool inRange) {
		...
		if (!playerDead && inRange && player.tag.Equals ("Player") && !inStasis && !inForceField && state != State.corrupted) {
	...
	public void PlayerSeenByOther () {
		if (state != State.dead && state != State.unconscious && !playerDead && !inStasis && !inForceField && state != State.corrupted) {
			suspicionPercentage = 1f;
			state = State.alerted;
			lastKnownLocation = player.transform.position;
			lastSeenTime = Time.time;
			transform.localScale = new Vector3 (Mathf.Abs (transform.localScale.x) * GetDirection (transform.position, player.transform.position), transform.localScale.y, 0);
			state = State.chasing;
            audioManager.PlaySound("Guard Surprise");
        }
	}
	...
	public void FoundGuard (Guard fellowGuard) {
		if (state != State.chasing && !playerDead && !inStasis && !inForceField && state != State.corrupted) {
			suspicionPercentage = 1f;
			transform.localScale = new Vector3 (Mathf.Abs (transform.localScale.x) * GetDirection (transform.position, fellowGuard.transform.position), transform.localScale.y, 0);
			guardFound = true;
	...
	void PlayerSeen () {
		if (state!=State.dead && !playerDead && !inStasis && !inForceField && state != State.corrupted) {
			...
			if (playerInMeleeRange) {
				...
				rb.velocity = new Vector2 (GetDirection (transform.position, player.transform.position) * runSpeed, 0f);
			}
			if (seesPlayer) {
				lastKnownLocation = player.transform.position;
				lastSeenTime = Time.time;
	...
	void SearchingForPlayer (Vector3 searchArea) {
		if (guardFound && Vector2.Distance (transform.position, searchArea) > 0.1f || Time.time < lastSeenTime + giveUpTime) {
			...
			if (!searchUnderway) {
				...
				if (oscillationsOccured < oscillationTimes.Length && Time.time > oscillationTimes [oscillationsOccured]) {
	...
	void AttackGuard (Transform fellowGuard) {
		if (state == State.corrupted && !playerDead && !inStasis && !inForceField) {
			if (transform.GetChild (0).GetComponent<CircleCollider2D> ().IsTouching (fellowGuard.GetComponent<BoxCollider2D> ())) {
				if (timeBetweenAttack <= 0) {
					...
					for (int i = 0; i < friendsToDamage.Length; i++) {
						if (friendsToDamage [i].transform != transform && friendsToDamage [i].name.Equals ("Guard Actual"))
				...
				transform.localScale = new Vector3 (Mathf.Abs (transform.localScale.x) * GetDirection (transform.position, closestGuard.position), transform.localScale.y, 0);
			}
		}
	}
	...
	void FindClosestGuard () {
		...
		if (fellowGuards.Count > 0) {
			for (int i = 0; i < fellowGuards.Count; i++) {
				...
				if ((closestGuardDistance == 0 || currentDistance < closestGuardDistance) && fellowGuardState != State.corrupted && fellowGuardState != State.unconscious && fellowGuardState != State.dead) {
	...
	void OnCollisionEnter2D (Collision2D collision) {
		if (collision.collider.tag.Equals ("Player") && !player.GetComponent<PlayerInfo> ().inShadowSink && !playerDead && !inStasis && !inForceField && state != State.corrupted) {
			suspicionPercentage = 1f;
			state = State.alerted;
			lastKnownLocation = player.transform.position;
			lastSeenTime = Time.time;
			...
			if ((collision.collider.gameObject.layer == 11 || collision.collider.gameObject.layer == 12 || collision.collider.gameObject.layer == 13) && maxVelocity >= 30f)
				GetComponent<LivingEntity> ().TakeHit (1000f);
			else if ((collision.collider.name.Contains ("Crate") || collision.collider.name.Contains ("Desk")) && maxVelocity >= 10f)
				OnGuardUnconscious ();
		}
	}
	...
	public void QuickLoad () {
		...
		transform.localScale = new Vector3 (guardQuickSave.direction, transform.localScale.y, transform.localScale.z);
