	...
	void Update () {
		...
		if (Input.GetAxis ("Use Item") == 1f && !stasisOccured) {
			...
			if (mouseRayHit) {
				...
				Debug.DrawRay (playerPosition, (targetPosition - playerPosition).normalized * Mathf.Clamp (Vector2.Distance (playerPosition, targetPosition), 0f, maxDistance), Color.white);
				...
				if (playerRayHit.collider == null) {
					if (Vector2.Distance (targetPosition, playerPosition) <= maxDistance) {
						...
						transform.position = playerPosition + (targetPosition - playerPosition).normalized * maxDistance;
					}
				} else {
