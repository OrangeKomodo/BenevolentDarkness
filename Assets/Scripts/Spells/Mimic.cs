			...
			if (mouseRayHit) {
				...
				if (playerRayHit.collider == null && !(mouseRayHit.collider.gameObject.layer == 9 && playerRayHit.collider.gameObject.layer == 9)) {
					...
					if (percentTransfered > 0)
						percentTransfered = 1f - ((Time.time - transferEndTime) / transferTime + (1f - startPercentage));
				...
				if (isTransfering)
					mimicMarker.transform.localScale = new Vector2 (1, 1) * ((markerSizeMax - markerSizeMin) * percentTransfered + markerSizeMin);
			}
		}

		if (Input.GetKeyDown (KeyCode.F) || Input.GetButtonDown ("Exit") || (Input.GetAxis ("Use Item") == 0f && !disguised))
